...
class BotaoCampo(private val campo: Campo): JButton() {
    ...
    private fun aplicarEstilo(campo: Campo, evento: CampoEvento) {
        when(evento) {
            CampoEvento.EXPOSAO -> aplicarEstiloExpodido()
    ...
    private fun aplicarEstiloExpodido() {
    ...
    private fun aplicarEstiloAberto() {
        ...
        foreground = when (campo.qtdDeVizinhosMinados) {
            1 -> COR_TXT_VERDE
            2 -> Color.BLUE
            3 -> Color.YELLOW
            4, 5, 6 -> Color.RED
        ...
        text = if (campo.qtdDeVizinhosMinados > 0) campo.qtdDeVizinhosMinados.toString() else ""
    }
